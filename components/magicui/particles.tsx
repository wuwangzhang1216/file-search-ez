...
export default function Particles({
  ...
  const mouseMovePosition = useRef<{ x: number; y: number }>({ x: 0, y: 0 });
  ...
  const onMouseMove = () => {
    if (canvasRef.current) {
      ...
      if (inside) {
        mouseMovePosition.current.x = x;
        mouseMovePosition.current.y = y;
      }
    }
  };
  ...
  const animate = () => {
    ...
    circles.current.forEach((circle: Circle, i: number) => {
      ...
      circle.translateX +=
        (mouseMovePosition.current.x / (staticity / circle.magnetism) -
          circle.translateX) /
        ease;
      circle.translateY +=
        (mouseMovePosition.current.y / (staticity / circle.magnetism) -
          circle.translateY) /
        ease;
